module "event_processing_dev" {
  source = "../../"
  
  # Development environment configuration
  environment                = "dev"
  location                   = "eastus"
  resource_group_name        = "event-proc-dev-rg"
  
  # AKS configuration - Development environment uses minimal resources
  aks_kubernetes_version     = "1.32.0"
  aks_node_count             = 1
  aks_vm_size                = "Standard_B2s"
  aks_auto_scaling_enabled   = true
  aks_auto_scaling_min_count = 1
  aks_auto_scaling_max_count = 2
